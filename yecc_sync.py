...
def _update_personal_info(parsed_data, resume_url, user_payload, lookups):
    try:
        ...
        res = requests.put(f"{YECC_BASE_URL}/ResumeBuilder/PersonalInfo/{resume_url}",
                           headers=YECC_HEADERS, json=personal_info_payload, timeout=30)
...
def _update_skills(parsed_data, resume_url, lookups):
    try:
        ...
        res = requests.put(f"{YECC_BASE_URL}/ResumeBuilder/ContactInfo/{resume_url}",
                           headers=YECC_HEADERS, json=payload, timeout=30)
        print(f"   Response: {res.status_code} {res.text[:200]}")
    except Exception as e:
...
def _update_experience(parsed_data, resume_url, lookups):
    try:
        ...
        res = requests.put(f"{YECC_BASE_URL}/ResumeBuilder/Experiences/{resume_url}",
                           headers=YECC_HEADERS, json=exps, timeout=30)
        print(f"   Response: {res.status_code} {res.text[:200]}")
    except Exception as e:
...
def _update_education(parsed_data, resume_url, lookups):
    """Update education in YECC (corrected structure)"""
    try:
        educations = []
    ...
    try:
        ...
        res = requests.put(f"{YECC_BASE_URL}/ResumeBuilder/EducationCertifications/{resume_url}",
                           headers=YECC_HEADERS, json=edus, timeout=30)
        print(f"   Response: {res.status_code} {res.text[:200]}")
    except Exception as e:
        print(f"   ⚠️ Education error: {e}")
...
def _update_certifications(parsed_data, resume_url):
    try:
        ...
        res = requests.put(f"{YECC_BASE_URL}/ResumeBuilder/Certifications/{resume_url}",
                           headers=YECC_HEADERS, json=certs, timeout=30)
        print(f"   Response: {res.status_code} {res.text[:200]}")
    except Exception as e:
